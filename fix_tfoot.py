...
old_tfoot = """          {sectionItems.length > 0 && (
            ...
            <tr>
              <td colSpan={3} className="p-4 text-right font-bold text-slate-500">總計 (年度等值)</td>
              <td className="p-4 text-right font-bold text-slate-900">${Math.floor(stats.yearlyTotal + stats.monthlyTotal).toLocaleString()}</td>
...
new_tfoot = """          {sectionItems.length > 0 && (() => {
            ...
            return (
              <tfoot className="bg-slate-50/80 border-t-2 border-slate-200">
              <tr>
                <td colSpan={3} className="p-4 text-right font-bold text-slate-500">小計 (年度等值)</td>
